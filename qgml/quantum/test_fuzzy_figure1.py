...
def test_generate_figure1ab_perturbed():
    """
    Generate plots similar to Figure 1a & 1b using the PERTURBED quantum metric (Eq. 7).
    (Fuzzy Sphere, noise=0.0).
    ...
    D = 3 # Embedding dimension for sphere
    true_dim = 2
    n_points_train = 2500 # REDUCED number of points for faster testing
    manifold_noise_std = 0.0 # Specific to this test (noiseless)

    # Parameters for noise=0 case (Using same as original 1ab for now)
    n_epochs = 100
    learning_rate = 0.01
    commutation_penalty = 0.0 # Note: This weight is NOT currently used in loss_for_backprop
    w_qf = 0.5 # Using w_qf = 0.0 for the noiseless case here as well
    batch_size = 500
    ...
    base_output_dir = Path("test_outputs/figure1_perturbed") # Separate output dir
    base_output_dir.mkdir(parents=True, exist_ok=True)
    ...
    # --- Start specific run for noise = 0.0 ---
    print(f"\n===== Running PERTURBED METRIC for manifold_noise_std = {manifold_noise_std} (Fig 1a/b) =====")
        start_time = time.time()

        # --- Directory Setup ---
    # Add perturbed suffix to distinguish runs
    params_str = f"N{N}_D{D}_pts{n_points_train}_noise{manifold_noise_std:.1f}_eps{n_epochs}_w{w_qf:.1f}_lr{learning_rate:.3f}_pen{commutation_penalty:.3f}_perturbed"
    output_dir = base_output_dir / params_str
    output_dir.mkdir(parents=True, exist_ok=True)
    print(f"Output directory: {output_dir}")

    # --- TensorBoard Setup ---
    # tensorboard_log_dir = output_dir / 'tensorboard_ab_perturbed' # Specific sub-dir
    # tensorboard_log_dir.mkdir(parents=True, exist_ok=True)
    # writer = SummaryWriter(log_dir=str(tensorboard_log_dir))
    ...
    # --- Plot Input Data (Fig 1a Equivalent) ---
    input_plot_filename = f"fig1_a_perturbed_input_points.png" # Added suffix
    plot_3d_points(
        train_points_tensor,
        colors=None,
        title=f"Input Training Data X (noise = {manifold_noise_std}) - Perturbed Test",
        save_path=output_dir / input_plot_filename
    )
    ...
    # *** USE PERTURBED METRIC CALCULATION ***
    print(f"Computing PERTURBED metrics for {n_points_train} training points...")
    metrics = estimator.compute_quantum_metrics_perturbed(train_points_tensor)
    # *******************************************

    print("Computing metric eigenvalues...")
    eigenvalues = estimator.compute_eigenspectrum(metrics)
    if eigenvalues is not None and eigenvalues.shape[1] > 2:
        print(f"Noise={manifold_noise_std}, Perturbed Eigenvalues shape: {eigenvalues.shape}, e2 min/max/mean: {eigenvalues[:, 2].min():.4f}/{eigenvalues[:, 2].max():.4f}/{eigenvalues[:, 2].mean():.4f}")

    # --- Plot Metric Eigenvalues (Fig 1b Equivalent) ---
    eigenvalue_filename = f"fig1_b_perturbed_metric_eigenvalues.png" # Added suffix
    plot_pointwise_eigenvalues(
        eigenvalues,
        output_dir=output_dir,
        filename=eigenvalue_filename
    ...
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"===== Completed PERTURBED run for manifold_noise_std = {manifold_noise_std} in {elapsed_time:.2f} seconds =====")

    # writer.close() # Close TensorBoard writer
    print(f"--- Finished test_generate_figure1ab_perturbed for noise={manifold_noise_std} ---")
...
def test_generate_figure1cd():
    ...
    n_epochs = 150
    learning_rate = 0.005
    commutation_penalty = 0.025 # Note: This weight is NOT currently used in loss_for_backprop
    w_qf = 0.1 
    batch_size = 500
