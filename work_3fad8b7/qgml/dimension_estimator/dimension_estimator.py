...
class DimensionEstimator:
    ...
    def _compute_quantum_metrics_tensor(self, points: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            # Convert input tensor `points` to NumPy for trainer.compute_eigensystem
            points_np_for_trainer = points.detach().cpu().numpy()
            Evals_batch_np, Evecs_batch_np = self.trainer.compute_eigensystem(points_np_for_trainer)

            # Convert results back to Tensors for further processing
            Evals_batch = torch.tensor(Evals_batch_np, dtype=torch.cfloat if Evals_batch_np.dtype == np.complex128 or Evals_batch_np.dtype == np.complex64 else torch.float32, device=device)
            Evecs_batch = torch.tensor(Evecs_batch_np, dtype=torch.cfloat if Evecs_batch_np.dtype == np.complex128 or Evecs_batch_np.dtype == np.complex64 else torch.float32, device=device)
    ...
    def compute_quantum_metrics(self, points_np: np.ndarray | None = None) -> torch.Tensor:
    ...
    def compute_eigenspectrum(self, points_np: np.ndarray | None = None) -> torch.Tensor | None:
        ...
        # Pass points_np (which can be None) to compute_quantum_metrics
        metrics_tensor = self.compute_quantum_metrics(points_np) 
