...
class MatrixConfigurationTrainer(nn.Module):
    ...
    def compute_eigensystem(self, points_np: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray]:
        ...
        Args:
            points_np (np.ndarray | None, optional): NumPy array of points with shape (batch_size, D).
                                                    If None, uses the points stored during initialization.
                                                    Defaults to None.

        Returns:
            A tuple (eigenvalues_batch_np, eigenvectors_batch_np) as NumPy arrays.
        """
        target_points_tensor: torch.Tensor
        if points_np is None:
            # Use internally stored points if no new points are provided
            if self.points is None: # self.points is the tensor version of self.points_np
                 raise ValueError("Trainer was not initialized with points, and no points were provided.")
            target_points_tensor = self.points
            print(f"--- Computing eigensystem for stored points ({target_points_tensor.shape[0]} points). ---")
        else:
            ...
            print(f"--- Computing eigensystem for provided {target_points_tensor.shape[0]} points. ---")

        # ensure the tensor is on the correct device (it should be if from self.points or converted)
        target_points_tensor = target_points_tensor.to(self.device)

        # call the internal method that works with tensors
        eigenvalues_batch_tensor, eigenvectors_batch_tensor = self._compute_eigensystem(target_points_tensor)

        # convert results to NumPy arrays
        eigenvalues_batch_np = eigenvalues_batch_tensor.detach().cpu().numpy()
        eigenvectors_batch_np = eigenvectors_batch_tensor.detach().cpu().numpy()
        
        return eigenvalues_batch_np, eigenvectors_batch_np
...
#            print(f"[Train Epoch] Batch points shape: {batch_points.shape}")
            ...
            # forward pass to compute loss components (now with guaranteed Hermitian matrices)
            ...
            # Note: Hermiticity projection now happens BEFORE forward pass in next iteration
