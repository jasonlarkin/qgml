...
class QGMLDimensionEstimator(BaseDimensionEstimator):
    ...
    def __init__(self, max_dim: int = 3, N: Optional[int] = None, output_dir: Optional[Path] = None):
        ...
        # Initialize matrix trainer
        self.trainer = MatrixConfigurationTrainer(N=self.N, D=self.D)
        
        # Initialize matrices
        self.matrices = self.trainer.generate_matrix_configuration()
    ...
    def compute_local_dimension(self, points: np.ndarray) -> np.ndarray:
        ...
        for i in range(n_points):
            try:
                ...
                g = self.trainer.compute_quantum_metric(self.matrices, point)
